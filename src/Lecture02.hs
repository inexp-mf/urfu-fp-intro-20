...
foldr f b Nil = b
foldr f b (x :. tail) = f x (foldr f b tail)
...
map f xs =  foldr (\e result -> (f e) :. result) Nil xs
...
filter f xs = foldr (\e result -> if f e then e :. result else result) Nil xs
...
foldl f b Nil = b
foldl f b (x :. tail) = foldl f (f b x) tail
...
reverse xs = foldl (\ys y -> y :. ys) Nil xs
...
-}
intercalate :: [a] -> [[a]] -> [a]
intercalate sep ls = P.foldr (\xs res -> if null res then xs else xs ++ sep ++ res) [] ls
